...
export class ShoppinghomeComponent {
  ...
  loadPopularProducts(): void {
    this.productApiService.getPopularProducts().subscribe({
      next: (res) => {
        this.popularProducts = this.mapProducts(res);
      },
      error: (err) => console.error('無法載入熱門商品', err)
    });
  }
  ...
  loadRecentlyViewed(): void {
    ...
    let viewedIds = [];
    try {
      viewedIds = JSON.parse(localStorage.getItem(key) || '[]');
    } catch (e) {
      viewedIds = [];
    }

    if (viewedIds && viewedIds.length > 0) {
      this.productApiService.getProductsByIds(viewedIds).subscribe({
        next: (res) => {
          this.recentlyViewed = this.mapProducts(res);
        },
        error: (err) => console.error('無法載入近期瀏覽', err)
      });
    }
  }
  ...
  onCategoryClick(category: any): void {
    ...
    this.router.navigate(['/shopping/products'], {
      queryParams: { keyword: category.keyword }
    });
  }
